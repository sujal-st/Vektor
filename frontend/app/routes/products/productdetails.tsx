    ...
    <>
      <section className='max-w-full mx-auto mt-10 p-5 flex flex-row items-start gap-16'>
        ...
        <div className='w-full flex flex-col justify-evenly gap-7'>
          <p className='text-gray-500'>Home / Products / <b>{product.title}</b></p>

          <div className='flex flex-col gap-2'>
            <h1 className='font-semibold text-4xl'>{product.title}</h1>
            <p className='text-xl text-gray-500 line-clamp-1'>{product.description}</p>
          </div>

          <p className='text-[#AB2320] font-semibold text-2xl'>Rs {Math.ceil(product.price * 145)}</p>

          <div className='grid grid-cols-2 gap-5'>

            <button className='px-4 py-2 bg-[#AB2320] text-[#F6F1E6] font-bold rounded-md cursor-pointer flex items-center justify-center gap-4'
              onClick={
                (e) => {
                  e.stopPropagation();
                  e.preventDefault();
                  isInCart ? removeFromCart(product.id, getToken())
                    : handleAddToCart(e)
                }
              }>
              <FaShoppingCart />
          ...
          <div className='border-t border-b grid grid-cols-3 gap-4 px-8 py-2 max-w-full'>
            {shippingRules.map((r) => (
              <div key={r.text} className='p-3 flex flex-col items-center rounded-lg self-center text-gray-600 text-sm'>
                {r.icon}
                {r.text}
              </div>
            ))}
